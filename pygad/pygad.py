...
class GA(
    utils.parent_selection.ParentSelection,
    ...
    supported_float_types = [float, numpy.float16, numpy.float32, numpy.float64, object]
        ...
        try:
            ...
            if type(gene_space) is type(None):
                    ...
                    for index, el in enumerate(gene_space):
                        if type(el) in [numpy.ndarray, list, tuple, range]:
                            if len(el) == 0:
                                ...
                                for val in el:
                                    if not (
                                        type(val) in [type(None)] + GA.supported_int_float_types
                                    ):
                                        raise TypeError(
                ...
                if len(gene_space.items()) == 2:
                    ...
                    if (
                        ("low" in gene_space.keys())
                        and ("high" in gene_space.keys())
                        and ("step" in gene_space.keys())
                    ):
                        pass
                    else:
                        self.valid_parameters = False
            ...
            if type(init_range_low) in GA.supported_int_float_types:
                if type(init_range_high) in GA.supported_int_float_types:
                    if init_range_low == init_range_high:
                        if not self.suppress_warnings:
                            warnings.warn(
                                "The values of the 2 parameters 'init_range_low' and 'init_range_high' are equal and this might return the same value for some genes in the initial population."
                            )
                else:
                    self.valid_parameters = False
                    raise TypeError(
                        f"Type mismatch between the 2 parameters 'init_range_low' {type(init_range_low)} and 'init_range_high' {type(init_range_high)}."
                    )
                ...
                if num_genes is None:
                    if initial_population is None:
                        ...
                        self.valid_parameters = False
                        raise ValueError(
                            f"The length of the 'init_range_low' parameter is {len(init_range_low)} which is different from the number of genes {len(initial_population[0])}."
                        )
                elif not len(init_range_low) == num_genes:
                    self.valid_parameters = False
                    raise ValueError(
                        f"The length of the 'init_range_low' parameter is {len(init_range_low)} which is different from the number of genes {num_genes}."
                    )
                ...
                if type(init_range_high) in [list, tuple, numpy.ndarray]:
                    if len(init_range_low) == len(init_range_high):
                        ...
                        self.valid_parameters = False
                        raise ValueError(
                            f"Size mismatch between the 2 parameters 'init_range_low' {len(init_range_low)} and 'init_range_high' {len(init_range_high)}."
                        )
                    ...
                    for val in init_range_low:
                        if type(val) in GA.supported_int_float_types:
                            ...
                            self.valid_parameters = False
                            raise TypeError(
                                f"When an iterable (list/tuple/numpy.ndarray) is assigned to the 'init_range_low' parameter, its elements must be numeric but the value {val} of type {type(val)} found."
                            )
                    ...
                    for val in init_range_high:
                        if type(val) in GA.supported_int_float_types:
                            ...
                            self.valid_parameters = False
                            raise TypeError(
                                f"When an iterable (list/tuple/numpy.ndarray) is assigned to the 'init_range_high' parameter, its elements must be numeric but the value {val} of type {type(val)} found."
                            )
                else:
                    self.valid_parameters = False
                    raise TypeError(
                        f"Type mismatch between the 2 parameters 'init_range_low' {type(init_range_low)} and 'init_range_high' {type(init_range_high)}. Both of them can be either numeric or iterable (list/tuple/numpy.ndarray)."
                    )
            else:
                self.valid_parameters = False
                raise TypeError(
                    f"The expected type of the 'init_range_low' parameter is numeric or list/tuple/numpy.ndarray but {type(init_range_low)} found."
                )
            ...
            # A single data type of float with precision.
                ...
                len(gene_type) == 2
                and gene_type[0] in GA.supported_float_types
                and (type(gene_type[1]) in GA.supported_int_types or gene_type[1] is None)
            ):
            ...
            # A single data type of integer with precision None ([int, None]).
                ...
                len(gene_type) == 2
                and gene_type[0] in GA.supported_int_types
                and gene_type[1] is None
            ):
                self.gene_type = gene_type
                self.gene_type_single = True
                        ...
                        if len(gene_type_val) == 2:
                            if gene_type_val[0] in GA.supported_float_types:
                                ...
                                if gene_type_val[1] is None:
                                    ...
                                    self.valid_parameters = False
                                    raise TypeError(
                                        f"In the 'gene_type' parameter, either do not set a precision for integer data types or set it to None. But the element {gene_type_val} at index {gene_type_idx} has a precision of {gene_type_val[1]} with type {gene_type_val[0]}."
                                    )
                            else:
                                self.valid_parameters = False
                                raise TypeError(
            ...
            if initial_population is None:
                ...
                self.valid_parameters = False
                raise ValueError(
                    f"A 2D list is expected to the initial_population parameter but a ({numpy.array(initial_population).ndim}-D) list found."
                )
            else:
                ...
                for row_idx in range(len(initial_population)):
                    for col_idx in range(len(initial_population[0])):
                        if (
                            type(initial_population[row_idx][col_idx])
                            in GA.supported_int_float_types
                        ):
                            pass
                        else:
                            self.valid_parameters = False
                ...
                if self.gene_type_single == True:
                    if self.gene_type[1] == None:
                        self.initial_population = numpy.array(
                            initial_population, dtype=self.gene_type[0]
                        )
                    else:
                    ...
                    self.initial_population = numpy.zeros(
                        shape=(initial_population.shape[0], initial_population.shape[1]),
                        dtype=object,
                    )
                    for gene_idx in range(initial_population.shape[1]):
                        if self.gene_type[gene_idx][1] is None:
                            self.initial_population[:, gene_idx] = numpy.asarray(
                                initial_population[:, gene_idx], dtype=self.gene_type[gene_idx][0]
                            )
                        else:
                            # This block is reached only for non-integer data types (i.e. float).
                ...
                if self.allow_duplicate_genes == False:
                    for initial_solution_idx, initial_solution in enumerate(
                        self.initial_population
                    ):
                        if self.gene_space is None:
                            self.initial_population[initial_solution_idx], _, _ = (
                                self.solve_duplicate_genes_randomly(
            ...
            if type(random_mutation_min_val) in GA.supported_int_float_types:
                if type(random_mutation_max_val) in GA.supported_int_float_types:
                    if random_mutation_min_val == random_mutation_max_val:
                        if not self.suppress_warnings:
                            warnings.warn(
                                "The values of the 2 parameters 'random_mutation_min_val' and 'random_mutation_max_val' are equal and this might cause a fixed mutation to some genes."
                            )
                else:
                    self.valid_parameters = False
                    raise TypeError(
                        f"Type mismatch between the 2 parameters 'random_mutation_min_val' {type(random_mutation_min_val)} and 'random_mutation_max_val' {type(random_mutation_max_val)}."
                    )
                ...
                if len(random_mutation_min_val) == self.num_genes:
                    ...
                    self.valid_parameters = False
                    raise ValueError(
                        f"The length of the 'random_mutation_min_val' parameter is {len(random_mutation_min_val)} which is different from the number of genes {self.num_genes}."
                    )
                if type(random_mutation_max_val) in [list, tuple, numpy.ndarray]:
                    if len(random_mutation_min_val) == len(random_mutation_max_val):
                        ...
                        self.valid_parameters = False
                        raise ValueError(
                            f"Size mismatch between the 2 parameters 'random_mutation_min_val' {len(random_mutation_min_val)} and 'random_mutation_max_val' {len(random_mutation_max_val)}."
                        )
                    ...
                    for val in random_mutation_min_val:
                        if type(val) in GA.supported_int_float_types:
                            ...
                            self.valid_parameters = False
                            raise TypeError(
                                f"When an iterable (list/tuple/numpy.ndarray) is assigned to the 'random_mutation_min_val' parameter, its elements must be numeric but the value {val} of type {type(val)} found."
                            )
                    ...
                    for val in random_mutation_max_val:
                        if type(val) in GA.supported_int_float_types:
                            ...
                            self.valid_parameters = False
                            raise TypeError(
                                f"When an iterable (list/tuple/numpy.ndarray) is assigned to the 'random_mutation_max_val' parameter, its elements must be numeric but the value {val} of type {type(val)} found."
                            )
                else:
                    self.valid_parameters = False
                    raise TypeError(
                ...
                self.valid_parameters = False
                raise TypeError(
                    f"The expected type of the 'random_mutation_min_val' parameter is numeric or list/tuple/numpy.ndarray but {type(random_mutation_min_val)} found."
                )
                            ...
                            if len(mutation_percent_genes) == 2:
                                ...
                                for idx, el in enumerate(mutation_percent_genes):
                                    ...
                                    if mutation_num_genes[idx] == 0:
                                        if not self.suppress_warnings:
                                            warnings.warn(
                                                f"The percentage of genes to mutate ({mutation_percent_genes[idx]}) resulted in selecting ({mutation_num_genes[idx]}) genes. The number of genes to mutate is set to 1 (mutation_num_genes=1).\nIf you do not want to mutate any gene, please set mutation_type=None."
                                            )
            ...
            if not (type(keep_parents) in GA.supported_int_types):
                ...
                keep_parents > self.sol_per_pop
                or keep_parents > self.num_parents_mating
                or keep_parents < -1
            ):
                self.valid_parameters = False
                raise ValueError(
            ...
            if stop_criteria is None:
                ...
                if len(criterion) == 2:
                    ...
                    if stop_word == "reach":
                        ...
                        self.valid_parameters = False
                        raise ValueError(
                            f"Passing multiple numbers following the keyword in the 'stop_criteria' parameter is expected only with the 'reach' keyword but the keyword ({stop_word}) found."
                        )
                    ...
                    for idx, num in enumerate(number):
                        try:
                            ...
                            self.valid_parameters = False
                            raise ValueError(
                                f"The value(s) following the stop word in the 'stop_criteria' parameter must be numeric but the value ({num}) of type {type(num)} found."
                            )
                ...
                if len(parallel_processing) == 2:
                    if type(parallel_processing[0]) is str:
                        if parallel_processing[0] in ["process", "thread"]:
                            if (
                                (
                                    type(parallel_processing[1]) in GA.supported_int_types
                                    and parallel_processing[1] > 0
                                )
                                or (parallel_processing[1] == 0)
                                or (parallel_processing[1] is None)
                            ):
        ...
        for gene_idx in range(self.num_genes):
            if self.gene_type_single:
                ...
                if not self.gene_type[gene_idx][1] is None:
                    solutions[:, gene_idx] = numpy.round(
                        numpy.asarray(solutions[:, gene_idx], dtype=self.gene_type[gene_idx][0]),
                        self.gene_type[gene_idx][1],
                    )
        return solutions

    def initialize_population(
        self, low, high, allow_duplicate_genes, mutation_by_replacement, gene_type
    ):
        """
        ...
        if self.gene_space is None:
            ...
            if self.gene_type_single == True:
                self.population = numpy.asarray(
                    numpy.random.uniform(low=low, high=high, size=self.pop_size),
                ...
                for gene_idx in range(self.num_genes):
                    ...
                    gene_values = numpy.asarray(
                        numpy.random.uniform(low=range_min, high=range_max, size=self.pop_size[0]),
                        dtype=self.gene_type[gene_idx][0],
                    )
                ...
                for sol_idx in range(self.sol_per_pop):
                    for gene_idx in range(self.num_genes):
                        ...
                        if self.gene_space[gene_idx] is None:
                            ...
                            self.population[sol_idx, gene_idx] = numpy.asarray(
                                numpy.random.uniform(low=range_min, high=range_max, size=1),
                                dtype=self.gene_type[0],
                            )[0]
                            ...
                            for idx, val in enumerate(self.gene_space[gene_idx]):
                                if val is None:
                                    self.gene_space[gene_idx][idx] = numpy.asarray(
                                        numpy.random.uniform(low=range_min, high=range_max, size=1),
                                        dtype=self.gene_type[0],
                                    )[0]
                            # Find the difference between the current gene space and the current values in the solution.
                            unique_gene_values = list(
                                set(self.gene_space[gene_idx]).difference(
                            ...
                            if "step" in self.gene_space[gene_idx].keys():
                                self.population[sol_idx, gene_idx] = numpy.asarray(
                                    numpy.random.choice(
                                        numpy.arange(
                                            start=self.gene_space[gene_idx]["low"],
                                ...
                                self.population[sol_idx, gene_idx] = numpy.asarray(
                                    numpy.random.uniform(
                                        low=self.gene_space[gene_idx]["low"],
            ...
            if self.gene_type_single == True:
                ...
                for gene_idx, curr_gene_space in enumerate(self.gene_space):
                    ...
                    if curr_gene_space is None:
                        self.gene_space[gene_idx] = numpy.asarray(
                            numpy.random.uniform(low=range_min, high=range_max, size=1),
                            dtype=self.gene_type[0],
                        )[0]
                ...
                if type(self.gene_space) is dict:
                    ...
                    self.population = numpy.asarray(
                        numpy.random.choice(self.gene_space, size=self.pop_size),
                        dtype=self.gene_type[0],
                    )  # A NumPy array holding the initial population.
            else:
                # Reaching this block means:
                ...
                if type(self.gene_space) is dict:
                    ...
                    for gene_idx in range(self.num_genes):
                        # A vector of all values of this single gene across all solutions in the population.
                        gene_values = numpy.asarray(
                            numpy.random.choice(self.gene_space, size=self.pop_size[0]),
                            dtype=self.gene_type[gene_idx][0],
                        )
                        # Adding the current gene values to the population.
                        self.population[:, gene_idx] = gene_values
            ...
            if self.parallel_processing is None:
                ...
                for sol_idx, sol in enumerate(self.population):
                    ...
                    if (
                        ...
                        and (list(sol) in self.solutions)
                    ):
                        solution_idx = self.solutions.index(list(sol))
                        fitness = self.solutions_fitness[solution_idx]
                    elif (
                        (self.save_best_solutions)
                        and (len(self.best_solutions) > 0)
                        and (list(sol) in self.best_solutions)
                    ):
                        solution_idx = self.best_solutions.index(list(sol))
                        fitness = self.best_solutions_fitness[solution_idx]
                    elif (
                        (self.keep_elitism > 0)
                        and (self.last_generation_elitism is not None)
                        and (len(self.last_generation_elitism) > 0)
                        and (list(sol) in last_generation_elitism_as_list)
                    ):
                    ...
                    # elif (self.last_generation_parents is not None) and len(numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0] > 0):
                        ...
                        ((self.keep_parents == -1) or (self.keep_parents > 0))
                        and (self.last_generation_parents is not None)
                        and (len(self.last_generation_parents) > 0)
                        and (list(sol) in last_generation_parents_as_list)
                    ):
                        ...
                        if self.fitness_batch_size in [1, None]:
                            ...
                            if type(fitness) in GA.supported_int_float_types:
                                ...
                                raise ValueError(
                                    f"The fitness function should return a number or an iterable (list, tuple, or numpy.ndarray) but the value {fitness} of type {type(fitness)} found."
                                )
                        else:
                ...
                if self.fitness_batch_size not in [1, None]:
                    ...
                    solutions_indices = [
                        idx
                        for idx, fit in enumerate(pop_fitness)
                        if type(fit) is str and fit == "undefined"
                    ]
                    # Number of batches.
                ...
                with ExecutorClass(max_workers=self.parallel_processing[1]) as executor:
                    ...
                    if self.fitness_batch_size in [1, None]:
                        ...
                        # Number of batches.
                        num_batches = int(
                            numpy.ceil(len(solutions_to_submit_indices) / self.fitness_batch_size)
                        )
                        ...
                        for batch_idx in range(num_batches):
                            batch_first_index = batch_idx * self.fitness_batch_size
                            batch_last_index = (batch_idx + 1) * self.fitness_batch_size
                            batch_indices = solutions_to_submit_indices[
                                batch_first_index:batch_last_index
                            ]
                            batch_solutions = self.population[batch_indices, :]
                        ...
                        for batch_indices, batch_fitness in zip(
                            batches_indices,
                            executor.map(
                                self.fitness_func,
                                [self] * len(solutions_to_submit_indices),
    ...
    def run(self):
        ...
        try:
            ...
            best_solution, best_solution_fitness, best_match_idx = self.very_best_solution()
            ...
            for generation in range(generation_first_idx, generation_last_idx):
                    ...
                    if on_fitness_output is None:
                        ...
                        if type(on_fitness_output) in [tuple, list, numpy.ndarray, range]:
                            ...
                            if on_fitness_output.shape == self.last_generation_fitness.shape:
                                ...
                                raise ValueError(
                                    f"Size mismatch between the output of on_fitness() {on_fitness_output.shape} and the expected fitness output {self.last_generation_fitness.shape}."
                                )
                        else:
                            raise ValueError(
                                f"The output of on_fitness() is expected to be tuple/list/range/numpy.ndarray but {type(on_fitness_output)} found."
                            )
                ...
                if not (self.on_generation is None):
                    ...
                    if type(r) is str and r.lower() == "stop":
                        ...
                        # _, best_solution_fitness, _ = self.best#solution()
                        self.best_solutions_fitness.append(best_solution_fitness)
                        break
                ...
                if not self.stop_criteria is None:
                    for criterion in self.stop_criteria:
                        if criterion[0] == "reach":
                            ...
                            if (
                                type(self.last_generation_fitness[0])
                                in GA.supported_int_float_types
                            ):
                            ...
                            # Multi-objective problem.
                            elif type(self.last_generation_fitness[0]) in [
                                list,
                                tuple,
                                numpy.ndarray,
                            ]:
                                ...
                                for obj_idx in range(len(self.last_generation_fitness[0])):
                                    ...
                                    if (
                                        max(self.last_generation_fitness[:, obj_idx])
                                        >= reach_fitness_value
                                    ):
                                        pass
                                    else:
            ...
            _, best_solution_fitness, _ = self.very_best_solution()
            self.best_solutions_fitness.append(best_solution_fitness)
    ...
    def run_select_parents(self, call_on_parents=True):
        ...
        if callable(self.parent_selection_type):
            self.last_generation_parents, self.last_generation_parents_indices = (
                self.select_parents(self.last_generation_fitness, self.num_parents_mating, self)
            )
        ...
        if call_on_parents:
            if not (self.on_parents is None):
                on_parents_output = self.on_parents(self, self.last_generation_parents)

                if on_parents_output is None:
                    pass
                elif type(on_parents_output) in [list, tuple, numpy.ndarray]:
                    if len(on_parents_output) == 2:
                        on_parents_selected_parents, on_parents_selected_parents_indices = (
                            on_parents_output
                        )
                    else:
                        raise ValueError(
                            f"The output of on_parents() is expected to be tuple/list/numpy.ndarray of length 2 but {type(on_parents_output)} of length {len(on_parents_output)} found."
                        )

                    # Validate the parents.
                    if on_parents_selected_parents is None:
                        raise ValueError(
                            "The returned outputs of on_parents() cannot be None but the first output is None."
                        )
                    else:
                        if type(on_parents_selected_parents) in [tuple, list, numpy.ndarray]:
                            ...
                            if (
                                on_parents_selected_parents.shape
                                == self.last_generation_parents.shape
                            ):
                                self.last_generation_parents = on_parents_selected_parents
                            else:
                                raise ValueError(
                                    f"Size mismatch between the parents retrned by on_parents() {on_parents_selected_parents.shape} and the expected parents shape {self.last_generation_parents.shape}."
                                )
                        else:
                            raise ValueError(
                                f"The output of on_parents() is expected to be tuple/list/numpy.ndarray but the first output type is {type(on_parents_selected_parents)}."
                            )

                    # Validate the parents indices.
                    if on_parents_selected_parents_indices is None:
                        raise ValueError(
                            "The returned outputs of on_parents() cannot be None but the second output is None."
                        )
                    else:
                        if type(on_parents_selected_parents_indices) in [
                            tuple,
                            list,
                            numpy.ndarray,
                            range,
                        ]:
                            on_parents_selected_parents_indices = numpy.array(
                                on_parents_selected_parents_indices
                            )
                            if (
                                on_parents_selected_parents_indices.shape
                                == self.last_generation_parents_indices.shape
                            ):
                                ...
                                raise ValueError(
                                    f"Size mismatch between the parents indices returned by on_parents() {on_parents_selected_parents_indices.shape} and the expected crossover output {self.last_generation_parents_indices.shape}."
                                )
                        else:
                            raise ValueError(
                                f"The output of on_parents() is expected to be tuple/list/range/numpy.ndarray but the second output type is {type(on_parents_selected_parents_indices)}."
                            )

                else:
                    raise TypeError(
                        f"The output of on_parents() is expected to be tuple/list/numpy.ndarray but {type(on_parents_output)} found."
                    )
    ...
    def run_crossover(self):
        ...
        if self.crossover_type is None:
            if self.keep_elitism == 0:
                num_parents_to_keep = (
                    self.num_parents_mating if self.keep_parents == -1 else self.keep_parents
                )
                if self.num_offspring <= num_parents_to_keep:
                    ...
                    self.last_generation_offspring_crossover = numpy.concatenate(
                        (
                            ...
                            self.population[
                                0 : (self.num_offspring - self.last_generation_parents.shape[0])
                            ],
                        )
                    )
            else:
                ...
                if self.num_offspring <= self.keep_elitism:
                    ...
                    self.last_generation_offspring_crossover = numpy.concatenate(
                        (
                            ...
                            self.population[
                                0 : (self.num_offspring - self.last_generation_elitism.shape[0])
                            ],
                        )
                    )
        else:
            ...
            if callable(self.crossover_type):
                self.last_generation_offspring_crossover = self.crossover(
                    self.last_generation_parents, (self.num_offspring, self.num_genes), self
                )
        ...
        if not (self.on_crossover is None):
            on_crossover_output = self.on_crossover(self, self.last_generation_offspring_crossover)
            if on_crossover_output is None:
                ...
                if type(on_crossover_output) in [tuple, list, numpy.ndarray]:
                    ...
                    if on_crossover_output.shape == self.last_generation_offspring_crossover.shape:
                        ...
                        raise ValueError(
                            f"Size mismatch between the output of on_crossover() {on_crossover_output.shape} and the expected crossover output {self.last_generation_offspring_crossover.shape}."
                        )
                else:
                    raise ValueError(
                        f"The output of on_crossover() is expected to be tuple/list/numpy.ndarray but {type(on_crossover_output)} found."
                    )
    ...
    def run_mutation(self):
        ...
        if not (self.on_mutation is None):
            on_mutation_output = self.on_mutation(self, self.last_generation_offspring_mutation)
            ...
            if on_mutation_output is None:
                ...
                if type(on_mutation_output) in [tuple, list, numpy.ndarray]:
                    ...
                    if on_mutation_output.shape == self.last_generation_offspring_mutation.shape:
                        ...
                        raise ValueError(
                            f"Size mismatch between the output of on_mutation() {on_mutation_output.shape} and the expected mutation output {self.last_generation_offspring_mutation.shape}."
                        )
                else:
                    raise ValueError(
                        f"The output of on_mutation() is expected to be tuple/list/numpy.ndarray but {type(on_mutation_output)} found."
                    )
    ...
    def run_update_population(self):
        ...
        if self.keep_elitism == 0:
            ...
            self.last_generation_elitism, self.last_generation_elitism_indices = (
    ...
    def best_solution(self, pop_fitness=None):
        ...
        try:
            ...
            try:
                best_match_idx = self.sort_solutions_nsga2(pop_fitness)[0]
            except Exception as ex:
                raise ex
    ...
    def summary(
        ...
        def create_row(
            columns, line_length=line_length, fill_character=fill_character, split_percentages=None
        ):
            filled_columns = []
            if split_percentages == None:
                split_percentages = [int(100 / len(columns))] * 3
            columns_lengths = [
                int((split_percentages[idx] * line_length) / 100)
        ...
        lifecycle_functions = [
            getattr(lifecycle_func, "__name__", "None") for lifecycle_func in lifecycle_functions
        ]
        lifecycle_functions = [
            lifecycle_func + "()" if lifecycle_func != "None" else "None"
            for lifecycle_func in lifecycle_functions
        ]
        lifecycle_output = [
            "None",
            "(1)",
            "None",
            f"({self.num_parents_mating}, {self.num_genes})",
            "None",
            f"({self.num_parents_mating}, {self.num_genes})",
            "None",
            f"({self.num_parents_mating}, {self.num_genes})",
        ...
        if not columns_equal_len:
            max_lengthes = [
                max(list(map(len, lifecycle_steps))),
                max(list(map(len, lifecycle_functions))),
                max(list(map(len, lifecycle_output))),
            ]
            split_percentages = [
        ...
        for lifecycle_idx in range(len(lifecycle_steps)):
            lifecycle_column = [
                lifecycle_steps[lifecycle_idx],
                lifecycle_functions[lifecycle_idx],
                lifecycle_output[lifecycle_idx],
            ]
    ...
    def very_best_solution(self) -> tuple:
        fronts = self.non_dominated_sorting(self.last_generation_fitness)[1]

        fitness: list = self.last_generation_fitness  # type: ignore

        nobj = len(fitness[0])
        # min in this case should be the most negative value
        # -> starting with values very close to 0
        min_fitness = [-1e-15] * nobj

        for i, _ in enumerate(fitness):
            for j in range(nobj):
                if fitness[i][j] > -1e15 and fitness[i][j] < min_fitness[j]:
                    min_fitness[j] = fitness[i][j]

        normed_fitness = numpy.zeros((len(fitness), nobj))
        for i, _ in enumerate(fitness):
            normed_fitness[i] = fitness[i] / min_fitness

        # get all solutions in the first front
        first_front_idx: list[int] = []
        for i, front in enumerate(fronts):
            if front == 0:
                first_front_idx.append(i)

        normalized_fitness_sum = numpy.sum(normed_fitness[first_front_idx, :], axis=1)
        solution_idx = numpy.argmin(normalized_fitness_sum)

        return (
            self.population[first_front_idx[solution_idx]],  # type: ignore
            self.last_generation_fitness[first_front_idx[solution_idx]],  # type: ignore
            first_front_idx[solution_idx],
        )
